...
import RightMessage from './RigthMessage'
...
const UserList = () => {
	...
	/*fetching users using redux hooks */
    const { allUsers } = useSelector((state) => {
	  ...
	  /*fetch users*/
	  useEffect(() => {
		pageno==1 && getAllusers(pageno)
	 }, [dispatch])
  
      const getAllusers=async(p)=>{
          await dispatch(getUsers(p))        
      }

	  /*auto scroll to bottom of the message*/
	...
	if(myRefTop.current.scrollIntoView && document.documentElement.scrollTop)return ;  
	setIsFetching(true);
	}

	/* fetchmore data on pagination*/
	async function fetchMoreListItems() {
		setTimeout(async()=>{
			await getAllusers(pageno+1)
			setIsFetching(true)
			setPageno(pageno+1)
			setIsFetching(false);
 ...
 {/* Header section of the messanger UI */}
            ...
            <header>
	<div id="idDetails">
		<div id={allUsers?.[allUsers.length-1]?.status==='active'?"imgCont":"imgContIn"}>
		</div>
		<div id="nameCont">
			<div id="name">{allUsers?.[allUsers?.length-1]?.name}</div>
			<div id="activeStat">
                {allUsers?.[allUsers?.length-1]?.status==='active'?"Active now":"Inactive Now"} 
            </div>
		</div>
	</div>
	<div id="chatOptions">
    ...
    { isFetching && <div style={{marginTop:10,textAlign:'center'}}>
	<div class="loader" style={{marginLeft:'auto',marginRight:'auto'}}></div>
	</div>}
...
<footer>
			...
			<div className="icon">
				<svg height="20px" width="20px" viewBox="0 -1 17 17">
					<g fill="none" fill-rule="evenodd">
						<path d="M2.882 13.13C3.476 4.743 3.773.48 3.773.348L2.195.516c-.7.1-1.478.647-1.478 1.647l1.092 11.419c0 .5.2.9.4 1.3.4.2.7.4.9.4h.4c-.6-.6-.727-.951-.627-2.151z" fill="#0084FF"></path>
						<circle cx="8.5" cy="4.5" r="1.5" fill="#0084FF"></circle>
						<path d="M14 6.2c-.2-.2-.6-.3-.8-.1l-2.8 2.4c-.2.1-.2.4 0 .6l.6.7c.2.2.2.6-.1.8-.1.1-.2.1-.4.1s-.3-.1-.4-.2L8.3 8.3c-.2-.2-.6-.3-.8-.1l-2.6 2-.4 3.1c0 .5.2 1.6.7 1.7l8.8.6c.2 0 .5 0 .7-.2.2-.2.5-.7.6-.9l.6-5.9L14 6.2z" fill="#0084FF"></path>
						<path d="M13.9 15.5l-8.2-.7c-.7-.1-1.3-.8-1.3-1.6l1-11.4C5.5 1 6.2.5 7 .5l8.2.7c.8.1 1.3.8 1.3 1.6l-1 11.4c-.1.8-.8 1.4-1.6 1.3z" stroke-linecap="round" stroke-linejoin="round" stroke="#0084FF"></path>
					</g>
				</svg>
			</div>
			<div className="icon">
				<svg height="20px" width="20px" viewBox="0 0 17 16" x="0px" y="0px">
					<g fill-rule="evenodd">
						...
						<path d="M10.4 13.1c0 .9-.4 1.6-.9 2.2 4.1-1.1 6.8-5.1 6.5-9.3-.4.6-1 1.1-1.8 1.5-2 1-3.7 3.6-3.8 5.6z" fill="#0084FF"></path>
						<path d="M2.5 13.4c.1.8.6 1.6 1.3 2 .5.4 1.2.6 1.8.6h.6l.4-.1c1.6-.4 2.6-1.5 2.7-2.9.1-2.4 2.1-5.4 4.5-6.6 1.3-.7 1.9-1.6 1.9-2.8l-.2-.9c-.1-.8-.6-1.6-1.3-2-.7-.5-1.5-.7-2.4-.5L3.6 1.5C1.9 1.8.7 3.4 1 5.2l1.5 8.2zm9-8.9c.6 0 1 .4 1 1s-.4 1-1 1-1-.4-1-1 .4-1 1-1zm-3.57 6.662c.3.1.4.4.3.6-.1.3-.3.4-.5.4h-.2c-1-.4-1.9-1.3-2.3-2.3-.1-.3.1-.6.3-.7.3-.1.5 0 .6.3.4.8 1 1.4 1.8 1.7zM5.5 5.5c.6 0 1 .4 1 1s-.4 1-1 1-1-.4-1-1 .4-1 1-1z" fill-rule="nonzero" fill="#0084FF"></path>
					</g>
				</svg>
			</div>
			<div className="icon">
				<svg height="20px" width="20px" viewBox="0 0 16 16" x="0px" y="0px">
					<path d="M.783 12.705c.4.8 1.017 1.206 1.817 1.606 0 0 1.3.594 2.5.694 1 .1 1.9.1 2.9.1s1.9 0 2.9-.1 1.679-.294 2.479-.694c.8-.4 1.157-.906 1.557-1.706.018 0 .4-1.405.5-2.505.1-1.2.1-3 0-4.3-.1-1.1-.073-1.976-.473-2.676-.4-.8-.863-1.408-1.763-1.808-.6-.3-1.2-.3-2.4-.4-1.8-.1-3.8-.1-5.7 0-1 .1-1.7.1-2.5.5s-1.417 1.1-1.817 1.9c0 0-.4 1.484-.5 2.584-.1 1.2-.1 3 0 4.3.1 1 .2 1.705.5 2.505zm10.498-8.274h2.3c.4 0 .769.196.769.696 0 .5-.247.68-.747.68l-1.793.02.022 1.412 1.252-.02c.4 0 .835.204.835.704s-.442.696-.842.696H11.82l-.045 2.139c0 .4-.194.8-.694.8-.5 0-.7-.3-.7-.8l-.031-5.631c0-.4.43-.696.93-.696zm-3.285.771c0-.5.3-.8.8-.8s.8.3.8.8l-.037 5.579c0 .4-.3.8-.8.8s-.8-.4-.8-.8l.037-5.579zm-3.192-.825c.7 0 1.307.183 1.807.683.3.3.4.7.1 1-.2.4-.7.4-1 .1-.2-.1-.5-.3-.9-.3-1 0-2.011.84-2.011 2.14 0 1.3.795 2.227 1.695 2.227.4 0 .805.073 1.105-.127V8.6c0-.4.3-.8.8-.8s.8.3.8.8v1.8c0 .2.037.071-.063.271-.7.7-1.57.991-2.47.991C2.868 11.662 1.3 10.2 1.3 8s1.704-3.623 3.504-3.623z" fill-rule="nonzero" fill="#0084FF"></path>
				</svg>
			</div>
		</div>
	</div>
	<div id="taCont">
		<div id="textarea" placeholder="Aa" contenteditable>
        </div>
		<div id="smily">
	...
	<div id="thumb" className="icon">
		<svg height="20px" width="20px" viewBox="0 0 24 24">
			<path d="M16.6915026,12.4744748 L3.50612381,13.2599618 C3.19218622,13.2599618 3.03521743,13.4170592 3.03521743,13.5741566 L1.15159189,20.0151496 C0.8376543,20.8006365 0.99,21.89 1.77946707,22.52 C2.41,22.99 3.50612381,23.1 4.13399899,22.8429026 L21.714504,14.0454487 C22.6563168,13.5741566 23.1272231,12.6315722 22.9702544,11.6889879 C22.8132856,11.0605983 22.3423792,10.4322088 21.714504,10.118014 L4.13399899,1.16346272 C3.34915502,0.9 2.40734225,1.00636533 1.77946707,1.4776575 C0.994623095,2.10604706 0.8376543,3.0486314 1.15159189,3.99121575 L3.03521743,10.4322088 C3.03521743,10.5893061 3.34915502,10.7464035 3.50612381,10.7464035 L16.6915026,11.5318905 C16.6915026,11.5318905 17.1624089,11.5318905 17.1624089,12.0031827 C17.1624089,12.4744748 16.6915026,12.4744748 16.6915026,12.4744748 Z" fill-rule="evenodd" stroke="none" fill="#0084FF"></path>
		</svg>
	</div>
</footer>
        </div>
